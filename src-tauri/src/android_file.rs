...
pub fn open_directory(path: &str) -> AppResult<()> {
    ...
    let dir_path = if p.is_dir() { path.to_string() } else {
        p.parent().map(|d| d.to_string_lossy().to_string()).unwrap_or(path.to_string())
    };

    // 创建 File 对象
    let file_cls = env.find_class("java/io/File").map_err(|e| {
        crate::error::AppError::Transfer(format!("File: {}", e))
    })?;
    let path_str = env.new_string(&dir_path).map_err(|e| {
        crate::error::AppError::Transfer(format!("new_string: {}", e))
    })?;
    let file_obj = env.new_object(&file_cls, "(Ljava/lang/String;)V", &[JValue::Object(&path_str)])
        .map_err(|e| crate::error::AppError::Transfer(format!("new File: {}", e)))?;

    // 通过 FileProvider 获取 content URI（兼容 Android 7+）
    let uri_cls = env.find_class("android/net/Uri").map_err(|e| {
        crate::error::AppError::Transfer(format!("Uri: {}", e))
    })?;
    let uri = env.call_static_method(&uri_cls, "fromFile", "(Ljava/io/File;)Landroid/net/Uri;", &[JValue::Object(&file_obj)])
        .and_then(|v| v.l())
        .map_err(|e| crate::error::AppError::Transfer(format!("Uri.fromFile: {}", e)))?;

    // 创建 Intent.ACTION_VIEW
    let intent_cls = env.find_class("android/content/Intent").map_err(|e| {
        crate::error::AppError::Transfer(format!("Intent: {}", e))
    })?;
    let action_view = env.get_static_field(&intent_cls, "ACTION_VIEW", "Ljava/lang/String;")
        .and_then(|v| v.l())
        .map_err(|e| crate::error::AppError::Transfer(format!("ACTION_VIEW: {}", e)))?;

    let intent = env.new_object(&intent_cls, "(Ljava/lang/String;)V", &[JValue::Object(&action_view)])
        .map_err(|e| crate::error::AppError::Transfer(format!("new Intent: {}", e)))?;

    // 设置 data + type 为资源目录
    let mime = env.new_string("resource/folder").map_err(|e| {
        crate::error::AppError::Transfer(format!("mime: {}", e))
    })?;
    env.call_method(&intent, "setDataAndType", "(Landroid/net/Uri;Ljava/lang/String;)Landroid/content/Intent;", &[JValue::Object(&uri), JValue::Object(&mime)])
        .map_err(|e| crate::error::AppError::Transfer(format!("setDataAndType: {}", e)))?;

    // 添加 FLAG_ACTIVITY_NEW_TASK（从非 Activity 上下文启动需要）
    let flag_new_task = env.get_static_field(&intent_cls, "FLAG_ACTIVITY_NEW_TASK", "I")
        .and_then(|v| v.i())
        .unwrap_or(0x10000000);
    env.call_method(&intent, "addFlags", "(I)Landroid/content/Intent;", &[JValue::Int(flag_new_task)])
        .map_err(|e| crate::error::AppError::Transfer(format!("addFlags: {}", e)))?;

    // 启动 Intent
    env.call_method(&context, "startActivity", "(Landroid/content/Intent;)V", &[JValue::Object(&intent)])
        .map_err(|e| crate::error::AppError::Transfer(format!("startActivity: {}", e)))?;

    tracing::info!("Android 打开目录: {}", dir_path);
    Ok(())
}
